"""Decide whether the source-serial backward should replace tiled backward.

This benchmark is intentionally separate from the accepted operator sweep. The
candidate is private and normal dispatch cannot select it. A result from this
file is evidence for a dispatch change, not a dispatch change by itself.
...
Run after the target GPU is available::
    ...
    python scripts/evaluate_backward.py results/backward_candidates_bfloat16.json
"""
...
def _gpu_preflight(device_index: int, *, allow_busy: bool) -> dict:
    ...
    processes = [line for line in processes if line.strip()]
    if processes and not allow_busy:
        raise SystemExit(
...
def _measure_trials(fn, *, device: torch.device, quick: bool) -> dict:
    """Store every timing sample and summarize the combined distribution."""
...
def bench_one(name, spec, shape, v, w, g, correctness_by_seed, device, quick) -> dict:
    """Measure one implementation after validating all returned gradients."""
    fn = spec["fn"]
    record = {
        "impl": name,
        "shape": asdict(shape),
    ...
    record["forward"] = _measure_trials(
        lambda: fn(v, w, DEFAULT_EPS),
        device=device,
        quick=quick,
    )

    # Keep one graph alive to time backward directly. This avoids subtracting
    # two noisy medians. ``retain_graph`` does not alter the custom backward
    # kernel; it only prevents Python autograd metadata from being released.
    ...
    record["backward"] = _measure_trials(
        backward_only,
        device=device,
        quick=quick,
    )
    try:
        record["backward_kernels"] = count_kernels(
            backward_only, device=device, iters=3
        ).as_dict()
    except Exception as exc:  # noqa: BLE001
    ...
    record["fwd_bwd"] = _measure_trials(
        fwd_bwd,
        device=device,
        quick=quick,
    )
    ...
    record["fwd_bwd_memory"] = measure_memory(
        ...
        output_bytes=shape.b * shape.t * shape.d * itemsize,
    ).as_dict()

    if name in {"current", "source_serial"}:
        n_pow2 = 1 << (shape.n - 1).bit_length()
        current_is_resident = _bwd_launch(n_pow2, shape.d)[0]
        model_name = (
            ("resident" if current_is_resident else "tiled")
            if name == "current"
            else "source_serial"
        )
...
def main() -> None:
    ...
    parser.add_argument("--impls", default="current,source_serial,liger")
    ...
    preflight = _gpu_preflight(args.device, allow_busy=args.allow_busy_gpu)
    ...
    report = {
        ...
        "experiment": "tiled backward source-serial candidate",
        ...
        "methodology": {
            ...
            "timing": "five trials of 40 CUDA-event samples; all raw samples stored",
    ...
    for shape_index, shape in enumerate(SHAPE_SETS[args.shape_set]):
        ...
        correctness: dict[str, list[dict]] = {name: [] for name in selected}
        ...
        available_order = [name for name in selected if name in implementations]
        ...
        report["execution_order"].append(
            {"shape": asdict(shape), "implementations": available_order}
        )
